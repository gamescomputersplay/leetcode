    ...
    def maxPoints(self, points):
        ...
        for i, point1 in enumerate(points):
            for point2 in points[:i]:
                ...
                shift = y1 if dy == 0 else x1 - (dx * y1) / dy

                # Track all possible lines and their points
                if (tilt, shift) not in lines:
                    lines[(tilt, shift)] = set()
                lines[(tilt, shift)].add(tuple(point1))
                lines[(tilt, shift)].add(tuple(point2))

        # Return max points along any line
